#!/usr/bin/env Rscript
# ============================================================
# Exos v2 — FindAllMarkers + 聚类热图 + 细胞类型注释
# 输入: seurat_exos_clustered_v2.rds (95,331 cells, 29 clusters)
# 输出目录: seurat5_exos_v2/
#   01_heatmap/  → FAM 热图 + DotPlot
#   02_annotation/ → 6大类注释 UMAP + 生殖细胞亚群分期
# ============================================================
...
H_DIR    <- file.path(EXOS_OUT, "01_heatmap")
...
write.csv(fam, file.path(H_DIR, "exos_v2_findallmarkers.csv"), row.names=FALSE)
...
p_heat <- DoHeatmap(
  ...
  labs(title=sprintf("Exos v2 — Top5 markers per cluster (%d clusters)", n_cl))

h_fig <- max(16, length(top_genes)*0.10 + 5)
ggsave(file.path(H_DIR,"exos_v2_heatmap.pdf"),  p_heat, width=26, height=h_fig)
ggsave(file.path(H_DIR,"exos_v2_heatmap.png"),  p_heat, width=26, height=h_fig, dpi=180)
cat("  Saved: exos_v2_heatmap.png\n")
rm(obj_sub); gc()
...
p_dot <- ggplot(dp_data, aes(x=features.plot, y=id,
  ...
  labs(x=NULL, y=NULL, title="Exos v2 — Marker DotPlot (all clusters)") +
...
ggsave(file.path(H_DIR,"exos_v2_dotplot.pdf"), p_dot, width=18, height=14)
ggsave(file.path(H_DIR,"exos_v2_dotplot.png"), p_dot, width=18, height=14, dpi=200)
cat("  Saved: exos_v2_dotplot.png\n")
...
p_s <- DimPlot(germ, reduction="umap", group.by="stage_label",
  ...
  labs(title="Exos v2 — Germ cells by stage") +
...
p_germ_all <- (p_s|p_c|p_g) / wrap_plots(p_feats,nrow=1) +
  ...
  plot_annotation(
    title="Exos v2 — Germ Cell Sub-clustering & Stage Annotation",
    theme=theme(plot.title=element_text(size=13,face="bold",hjust=0.5),
                plot.background=element_rect(fill="white"))
  )

ggsave(file.path(A_DIR,"exos_v2_germcell_overview.pdf"), p_germ_all, width=18,height=12)
ggsave(file.path(A_DIR,"exos_v2_germcell_overview.png"), p_germ_all, width=18,height=12,dpi=200)
cat("  Saved: exos_v2_germcell_overview.png\n")
...
dp_st <- dp_raw %>%
  ...
  summarise(avg.exp.scaled=mean(avg.exp.scaled,na.rm=TRUE),
            pct.exp=mean(pct.exp,na.rm=TRUE), .groups="drop") %>%
  group_by(features.plot) %>%
  mutate(exp_norm={
    rng <- range(avg.exp.scaled, na.rm=TRUE)
    if (diff(rng)<1e-6) rep(0,n())
...
p_gdot <- ggplot(dp_st, aes(x=features.plot,y=stage,size=pct.exp,color=exp_norm)) +
  ...
  scale_y_discrete(position="right") +
  labs(x=NULL,y=NULL,title="Exos v2 — Germ cells by stage") +
...
ggsave(file.path(A_DIR,"exos_v2_germcell_dotplot.pdf"), p_gdot_full, width=15,height=7)
ggsave(file.path(A_DIR,"exos_v2_germcell_dotplot.png"), p_gdot_full, width=15,height=7,dpi=200)
cat("  Saved: exos_v2_germcell_dotplot.png\n")

# C: 全局 UMAP（含体细胞）
p_umap_broad <- DimPlot(obj, reduction="umap", label=TRUE, label.size=3.5,
                         pt.size=0.15) +
  labs(title=sprintf("Exos v2 — %d clusters", n_cl)) +
  theme_cowplot(font_size=11) +
  theme(plot.title=element_text(hjust=0.5,face="bold"),
        plot.background=element_rect(fill="white"),
        legend.position="none")
...
p_umap_all <- (p_umap_broad | p_umap_grp) +
  plot_annotation(title="Exos v2 — All cells",
                  theme=theme(plot.title=element_text(size=13,face="bold",hjust=0.5),
                              plot.background=element_rect(fill="white")))

ggsave(file.path(A_DIR,"exos_v2_umap_all.png"), p_umap_all, width=14,height=6,dpi=200)
cat("  Saved: exos_v2_umap_all.png\n")

# ══════════════════════════════════════════════════════════════
# STEP 7: 保存
# ══════════════════════════════════════════════════════════════
cat("\n=== Saving ===\n")
saveRDS(germ, file.path(EXOS_OUT,"rds","seurat_exos_germcells_v2.rds"))
write.csv(
  data.frame(cluster=names(germ_stage), stage=germ_stage),
  file.path(A_DIR,"germ_stage_annotation.csv"), row.names=FALSE
)
write.csv(
  data.frame(cluster=cluster_ids, celltype=cluster_ann),
  file.path(H_DIR,"broad_cluster_annotation.csv"), row.names=FALSE
)
cat("  Saved: seurat_exos_germcells_v2.rds\n")

cat("\n===== DONE =====\n")
cat("Outputs:\n")
cat(sprintf("  %s/\n", EXOS_OUT))
cat("  01_heatmap/ → exos_v2_heatmap.png, exos_v2_dotplot.png\n")
cat("  02_annotation/ → exos_v2_germcell_overview.png, exos_v2_germcell_dotplot.png\n")
cat("  rds/seurat_exos_germcells_v2.rds\n")
